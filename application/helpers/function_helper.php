...
function admin_template2($view, $data)
{
	$ci = get_instance();
	$ci->load->view('_includes/admin/header2.php', $data);
	$ci->load->view($view);
	$ci->load->view('_includes/admin/footer.php');
}
...
function open_menu($page)
{
	$ci = get_instance();
	$uri = $ci->uri->segment(1);

	return $uri == $page ? "menu-open" : "";
}
...
{
	if (!userdata()) {
		setMsg("danger", "<strong>Akses Ditolak!</strong> Sign-in untuk masuk ke dashboard.");
		redirect('signin');
	}
}
